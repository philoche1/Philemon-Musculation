...
async function sendWelcomeEmail(req, res) {
  ...
  if (!name || !email || !pin) return res.status(400).json({ error: 'Champs manquants' });
  try {
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 500px; margin: auto;">
            ...
            <div style="background:#F2F4F7; border:1px solid #E5E7EB; border-radius:8px; padding:12px 14px; margin-bottom:18px; font-family: monospace; font-size:16px; word-break:break-all;">${email}</div>
            <p style="margin-bottom:6px;"><strong>Code d'accès</strong></p>
            <div style="background:#F2F4F7; border:1px solid #E5E7EB; border-radius:8px; padding:12px 14px; margin-bottom:18px; font-family: monospace; font-size:20px; letter-spacing:4px; text-align:center;">${pin}</div>
            ...
            <a href="${process.env.WELCOME_VIDEO_URL_MUSCU || '#'}" style="display:block; text-decoration:none; text-align:center;">
              <img src="https://suivi.philemon-musculation.com/philemon-video-thumbnail.png?v=4" alt="Voir la vidéo de bienvenue" width="280" height="326" style="display:inline-block; width:280px; height:326px; max-width:100%; border-radius:8px; border:0;" />
            </a>
          </div>
        `,
      }),
...
async function sendPhotoNotification(req, res) {
  const authHeader = req.headers.authorization || '';
  const token = authHeader.replace('Bearer ', '');
...
export default async function handler(req, res) {
  ...
  const { type } = req.body || {};
  if (type === 'photo') return sendPhotoNotification(req, res);
  return sendWelcomeEmail(req, res);
}
